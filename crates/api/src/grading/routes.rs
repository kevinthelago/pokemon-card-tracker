...
async fn verify_instance(
    ...
    if workspace_id.is_none() {
        return Err(AppError::NotFound(format!("card instance {instance_id}")));
    }
    ...
    super::cache::update_instance_status(&state.db, instance_id, &outcome).await?;
...
fn validate_cert_number(cert: &str) -> Result<(), AppError> {
    if cert.trim().is_empty() {
        return Err(AppError::Validation("cert_number must not be empty".into()));
    }
    if cert.len() > 64 {
